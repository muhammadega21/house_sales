...
@section('title', 'Edit Pengajuan KPR')

@section('content')
    <div class="space-y-6" x-data="pengajuanKprForm()">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Edit Pengajuan KPR</h1>
                <p class="text-sm text-gray-500">Perbarui data pengajuan KPR untuk booking ini.</p>
            </div>
            <a href="{{ route('marketing.pengajuan-kpr.show', $pengajuan->id) }}"
        ...
        <form action="{{ route('marketing.pengajuan-kpr.update', $pengajuan->id) }}" method="POST">
            @csrf
            @method('PUT')

            <x-card title="Booking" subtitle="Booking yang diajukan KPR">
            ...
            <x-card title="Detail Pengajuan KPR" subtitle="Perbarui field pengajuan">
                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="nama_bank" id="nama_bank" value="{{ old('nama_bank', $pengajuan->nama_bank) }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="plafon_kpr" id="plafon_kpr" min="0" step="1000" value="{{ old('plafon_kpr', $pengajuan->plafon_kpr) }}"
                            class="w-full rounded-lg border border-gray-300 py-2.5 text-sm transition focus:border-primary focus:ring-primary">
                        @error('plafon_kpr')
                            <p class="mt-1 text-xs text-red-600 font-semibold">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="tenor_tahun" id="tenor_tahun" min="1" max="30" value="{{ old('tenor_tahun', $pengajuan->tenor_tahun) }}"
                            class="w-full rounded-lg border border-gray-300 py-2.5 text-sm transition focus:border-primary focus:ring-primary">
                        @error('tenor_tahun')
                            <p class="mt-1 text-xs text-red-600 font-semibold">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="suku_bunga" id="suku_bunga" min="0" step="0.01" value="{{ old('suku_bunga', $pengajuan->suku_bunga) }}"
                            class="w-full rounded-lg border border-gray-300 py-2.5 text-sm transition focus:border-primary focus:ring-primary">
                        @error('suku_bunga')
                            <p class="mt-1 text-xs text-red-600 font-semibold">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_pengajuan" id="tanggal_pengajuan" value="{{ old('tanggal_pengajuan', $pengajuan->tanggal_pengajuan?->toDateString()) }}"
                            class="w-full rounded-lg border border-gray-300 py-2.5 text-sm transition focus:border-primary focus:ring-primary">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="catatan" id="catatan" rows="4"
                            class="w-full rounded-lg border border-gray-300 py-2.5 text-sm transition focus:border-primary focus:ring-primary">{{ old('catatan', $pengajuan->catatan) }}</textarea>
                        @error('catatan')
                            <p class="mt-1 text-xs text-red-600 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
