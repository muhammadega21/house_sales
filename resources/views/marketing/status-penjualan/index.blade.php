...
@section('title', 'Status Penjualan Saya')

@section('content')
    <div class="space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Status Penjualan Saya</h1>
                <p class="mt-2 text-sm text-gray-600">Daftar status penjualan milik Anda.</p>
            </div>
        </div>

        <x-card title="Filter">
            <form method="GET" action="{{ route('marketing.status-penjualan.index') }}" class="grid gap-4 lg:grid-cols-3">
                <div>
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        @forelse($statusPenjualans as $index => $statusPenjualan)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap"><x-badge :status="$statusPenjualan->status_saat_ini" /></td>
                                <td class="px-4 py-3 whitespace-nowrap text-gray-700">
                                    {{ $statusPenjualan->tanggal_perubahan?->format('d/m/Y H:i') ?? '-' }}</td>
                                <td class="px-4 py-3 whitespace-nowrap text-sm font-medium">
                                    <a href="{{ route('marketing.status-penjualan.show', $statusPenjualan->id) }}"
                                        class="text-primary hover:text-primary-dark">Detail</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="px-4 py-6 text-center text-sm text-gray-500">Tidak ada data status
                                    penjualan.</td>
                            </tr>
                        @endforelse
                    </tbody>
