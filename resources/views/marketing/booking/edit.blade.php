    ...
    @if($booking->statusPenjualan !== 'booking')
        <div class="rounded-lg bg-amber-50 border border-amber-200 p-4">
            <div class="flex items-center gap-3">
                ...
                <p class="text-sm font-semibold text-amber-800">Booking hanya bisa diedit jika status penjualan masih 'booking'. Status saat ini: {{ \App\Enums\StatusPenjualan::tryFrom($booking->statusPenjualan)?->label() ?? $booking->statusPenjualan }}</p>
            </div>
        </div>
    @endif

    <form action="{{ route('marketing.booking.update', $booking->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <x-card title="Detail Booking">
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    <label for="id_konsumen" class="block text-sm font-semibold text-gray-700 mb-1">Konsumen <span class="text-red-500">*</span></label>
                    <select name="id_konsumen" id="id_konsumen"
                        ...
                        @foreach($konsumenOptions as $id => $label)
                            <option value="{{ $id }}" {{ old('id_konsumen', $booking->id_konsumen) == $id ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('id_konsumen')
                ...
                <div>
                    <label for="id_unit" class="block text-sm font-semibold text-gray-700 mb-1">Unit Rumah <span class="text-red-500">*</span></label>
                    <select name="id_unit" id="id_unit"
                        {{ $booking->statusPenjualan !== 'booking' ? 'disabled' : '' }}
                        class="w-full rounded-lg border border-gray-300 py-2.5 text-sm transition focus:border-primary focus:ring-primary {{ $booking->statusPenjualan !== 'booking' ? 'bg-gray-50' : '' }}">
                        <option value="">-- Pilih Unit --</option>
                        @foreach($unitOptions as $id => $label)
                            <option value="{{ $id }}" {{ old('id_unit', $booking->id_unit) == $id ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('id_unit')
                        <p class="mt-1 text-xs text-red-600 font-semibold">{{ $message }}</p>
                ...
                <div>
                    <label for="tanggal_booking" class="block text-sm font-semibold text-gray-700 mb-1">Tanggal Booking</label>
                ...
                <div>
                    <label for="booking_fee" class="block text-sm font-semibold text-gray-700 mb-1">Booking Fee (Rp)</label>
                    <input type="number" name="booking_fee" id="booking_fee"
                        value="{{ old('booking_fee', $booking->booking_fee) }}"
                        min="0" step="1000"
                        {{ $booking->statusPenjualan !== 'booking' ? 'disabled' : '' }}
                        class="w-full rounded-lg border border-gray-300 py-2.5 text-sm transition focus:border-primary focus:ring-primary {{ $booking->statusPenjualan !== 'booking' ? 'bg-gray-50' : '' }}">
                    @error('booking_fee')
                ...
                <div>
                    <label for="metode_bayar_fee" class="block text-sm font-semibold text-gray-700 mb-1">Metode Bayar</label>
                    <select name="metode_bayar_fee" id="metode_bayar_fee"
                        ...
                        @foreach(\App\Enums\MetodeBayar::cases() as $metode)
                            @if($metode !== \App\Enums\MetodeBayar::Kpr)
                                <option value="{{ $metode->value }}" {{ old('metode_bayar_fee', $booking->metode_bayar_fee) === $metode->value ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    @if($booking->bukti_bayar_fee)
                        <div class="flex items-center gap-2">
                            ...
                            <a href="{{ asset('storage/' . $booking->bukti_bayar_fee) }}" target="_blank" class="text-sm text-primary hover:text-primary-dark">Lihat</a>
                        </div>
                    @else
                ...
                <div>
                    <label for="bukti_bayar_fee" class="block text-sm font-semibold text-gray-700 mb-1">Ganti Bukti Bayar</label>
                    <x-form-file name="bukti_bayar_fee" label="Upload Bukti Bayar (opsional)" accept="image/jpeg,image/png,image/webp,application/pdf" />
                ...
                <div class="md:col-span-2">
                    ...
                    <x-form-textarea name="catatan" rows="3" placeholder="Catatan tambahan (opsional)" :value="old('catatan', $booking->catatan)" />
                    @error('catatan')
                        <p class="mt-1 text-xs text-red-600 font-semibold">{{ $message }}</p>
                    @enderror
                </div>
