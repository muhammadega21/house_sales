    ...
    <div class="space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">Kelola data konsumen yang Anda input.</p>
            </div>
            <a href="{{ route('marketing.konsumen.create') }}"
        ...
        <x-data-table-toolbar
            ...
            :search-route="route('marketing.konsumen.index')"
            :per-page="$perPage ?? 10"
            :total="0"
            :filtered="0"
            :search="$search ?? ''"
                            ...
                            <tr class="transition hover:bg-gray-50">
                                ...
                                <td class="px-4 py-4 text-right">
                                    <div class="inline-flex items-center gap-3">
                                        ...
                                        <x-confirm-delete :route="route('marketing.konsumen.destroy', $konsumen->id)" :item-name="$konsumen->nama_lengkap" />
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7">
                                    <x-empty-state title="Belum ada konsumen"
                                        message="Mulai tambahkan konsumen baru untuk memudahkan proses booking."
                                        :search="$search ?? ''" :create-route="route('marketing.konsumen.create')"
                                        create-label="Tambah Konsumen" :reset-route="route('marketing.konsumen.index')" />
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
