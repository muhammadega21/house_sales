    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                <div class="h-16 w-16 rounded-full bg-gray-200 overflow-hidden flex items-center justify-center">
                    @if ($konsumen->foto_ktp)
                        <img src="{{ asset('storage/' . $konsumen->foto_ktp) }}" alt="Foto KTP"
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('marketing.konsumen.edit', $konsumen->id) }}"
                ...
                <a href="{{ route('marketing.dokumen.index', $konsumen->id) }}"
                    ...
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M4 16l4.586-4.586a2 2 0 012.828 0L13 13m-3-3l2.586-2.586a2 2 0 012.828 0L20 12" />
                    </svg>
                    Dokumen KPR
                </a>
                <a href="{{ route('marketing.booking.create') }}?id_konsumen={{ $konsumen->id }}"
        ...
        <x-card>
            <div class="border-b border-gray-200">
                <nav class="flex gap-6 px-6" id="tabs">
                    ...
                    <a href="?tab=data"
                        class="pb-2 text-sm font-semibold border-b-2 transition {{ $activeTab === 'data' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700' }}">Data
                        Pribadi</a>
                    <a href="?tab=dokumen"
                        class="pb-2 text-sm font-semibold border-b-2 transition {{ $activeTab === 'dokumen' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700' }}">Dokumen</a>
                    <a href="?tab=booking"
                        class="pb-2 text-sm font-semibold border-b-2 transition {{ $activeTab === 'booking' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700' }}">Riwayat
                        Booking</a>
                    <a href="?tab=dokumen-kpr"
                        class="pb-2 text-sm font-semibold border-b-2 transition {{ $activeTab === 'dokumen-kpr' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700' }}">Dokumen
                        KPR</a>
                    <a href="?tab=pengajuan-kpr"
                        class="pb-2 text-sm font-semibold border-b-2 transition {{ $activeTab === 'pengajuan-kpr' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700' }}">Pengajuan
                        KPR</a>
                </nav>
            </div>
                ...
                @if ($activeTab === 'data')
                    <div class="grid gap-x-6 md:grid-cols-2">
                        <div>
                            ...
                            <p class="font-semibold">{{ $konsumen->nama_lengkap }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400 uppercase">NIK</p>
                            <p class="font-semibold font-mono">{{ $konsumen->nik }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400 uppercase">No. KK</p>
                            <p class="font-semibold">{{ $konsumen->no_kk ?? '-' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400 uppercase">No. HP</p>
                            <p class="font-semibold">{{ $konsumen->no_hp }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400 uppercase">Email</p>
                            <p class="font-semibold">{{ $konsumen->email ?? '-' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400 uppercase">Alamat</p>
                            <p class="font-semibold">{{ $konsumen->alamat_lengkap }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400 uppercase">Tempat Lahir</p>
                            <p class="font-semibold">{{ $konsumen->tempat_lahir ?? '-' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400 uppercase">Tanggal Lahir</p>
                            <p class="font-semibold">{{ $konsumen->tanggal_lahir?->format('d/m/Y') ?? '-' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400 uppercase">Jenis Kelamin</p>
                            <p class="font-semibold">{{ $konsumen->jenis_kelamin?->label() ?? '-' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400 uppercase">Status Pernikahan</p>
                            <p class="font-semibold">{{ $konsumen->status_pernikahan?->label() ?? '-' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400 uppercase">Pekerjaan</p>
                            <p class="font-semibold">{{ $konsumen->pekerjaan ?? '-' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400 uppercase">Nama Perusahaan</p>
                            <p class="font-semibold">{{ $konsumen->nama_perusahaan ?? '-' }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400 uppercase">Penghasilan Bulanan</p>
                            <p class="font-semibold">
                                {{ $konsumen->penghasilan_bulanan ? 'Rp ' . number_format($konsumen->penghasilan_bulanan, 0, ',', '.') : '-' }}
                            </p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400 uppercase">NPWP</p>
                            <p class="font-semibold">{{ $konsumen->npwp ?? '-' }}</p>
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            @if ($konsumen->foto_ktp)
                                <a href="{{ asset('storage/' . $konsumen->foto_ktp) }}" target="_blank">
                                    <img src="{{ asset('storage/' . $konsumen->foto_ktp) }}" alt="KTP"
                                        class="rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow cursor-pointer max-h-64 object-contain">
                                </a>
                            @else
                        ...
                        <div>
                            ...
                            @if ($konsumen->foto_kk)
                                <a href="{{ asset('storage/' . $konsumen->foto_kk) }}" target="_blank">
                                    <img src="{{ asset('storage/' . $konsumen->foto_kk) }}" alt="KK"
                                        class="rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow cursor-pointer max-h-64 object-contain">
                                </a>
                            @else
                            ...
                            @foreach ($konsumen->bookings as $booking)
                                <x-card>
                                    <div class="mb-3 flex items-center justify-between">
                                        <div>
                                            <p class="text-sm font-semibold text-gray-900">{{ $booking->kode_booking }}
                                            </p>
                                            <p class="text-xs text-gray-500">{{ $booking->unit?->kode_unit ?? '-' }} ·
                                                {{ $booking->unit?->tipe_rumah ?? '' }}</p>
                                        </div>
                                ...
                                <tbody class="divide-y divide-gray-100 bg-white">
                                    @foreach ($konsumen->pengajuanKpr as $pengajuan)
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-4 py-3">{{ $pengajuan->nama_bank ?? '-' }}</td>
                                            <td class="px-4 py-3">Rp
                                                {{ number_format($pengajuan->plafon_kpr ?? 0, 0, ',', '.') }}</td>
                                            <td class="px-4 py-3">{{ $pengajuan->tenor_tahun ?? '-' }} tahun</td>
                                            <td class="px-4 py-3">{{ __($pengajuan->status_pengajuan) }}</td>
                                            <td class="px-4 py-3">{{ $pengajuan->tanggal_pengajuan?->format('d/m/Y') }}
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
