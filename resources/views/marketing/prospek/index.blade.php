    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">Kelola prospek dan follow-up calon konsumen.</p>
            </div>
            <a href="{{ route('marketing.prospek.create') }}"
        ...
        {{-- Data Table Toolbar --}}
        <x-data-table-toolbar search-placeholder="Cari nama atau no HP prospek..." :search-route="route('marketing.prospek.index')" :per-page="$perPage"
            :total="$stats['total'] ?? 0" :filtered="$prospeks->total()" :search="$search" :has-filters="$hasFilters"
            :exclude-keys="['status_prospek', 'sumber_prospek']">
            <x-slot:filters>
                        ...
                        <option value="">Semua Status</option>
                         @foreach (\App\Enums\StatusProspek::cases() as $status)
                            <option value="{{ $status->value }}"
                                {{ request('status_prospek') === $status->value ? 'selected' : '' }}>
                                {{ $status->label() }}
                            </option>
                        @endforeach
                    </select>
                        ...
                        <option value="">Semua Sumber</option>
                         @foreach (\App\Enums\SumberProspek::cases() as $sumber)
                            <option value="{{ $sumber->value }}"
                                {{ request('sumber_prospek') === $sumber->value ? 'selected' : '' }}>
                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        @forelse($prospeks as $prospek)
                            <tr class="transition hover:bg-gray-50">
                                ...
                                <td class="px-4 py-4">
                                    ...
                                    @if ($sumber)
                                        <span
                                            class="inline-flex items-center gap-1.5 rounded-md bg-gray-100 px-2.5 py-1 text-xs font-medium text-gray-800 border border-gray-200">
                                ...
                                <td class="px-4 py-4 text-gray-700 whitespace-nowrap">
                                    {{ $prospek->tanggal_prospek->format('d/m/Y') }}</td>
                                <td class="px-4 py-4 text-gray-700 max-w-[200px]">
                                    <div class="truncate" title="{{ $prospek->catatan ?: '-' }}">
                                        {{ $prospek->catatan ?: '-' }}</div>
                                </td>
                                    ...
                                    <div class="inline-flex items-center gap-3">
                                        ...
                                        @if ($prospek->status_prospek->value === 'berminat')
                                            <a href="{{ route('marketing.prospek.convert', $prospek->id) }}"
                                                class="font-semibold text-emerald-600 transition hover:text-emerald-800"
                                                title="Konversi ke Konsumen">
                                        ...
                                        @if ($prospek->status_prospek->value !== 'jadi_konsumen')
                                            <x-confirm-delete :route="route('marketing.prospek.destroy', $prospek->id)" :item-name="$prospek->nama_prospek" />
                                        @endif
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="8">
                                    <x-empty-state title="Belum ada prospek"
                                        message="Mulai tambahkan prospek baru untuk memudahkan follow-up calon konsumen."
                                        :search="$search" :create-route="route('marketing.prospek.create')" create-label="Tambah Prospek"
                                        :reset-route="route('marketing.prospek.index')" />
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
