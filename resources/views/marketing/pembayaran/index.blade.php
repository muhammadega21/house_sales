            ...
            <div class="min-w-[180px]">
                ...
                <select name="id_booking" id="id_booking" onchange="this.closest('form').submit()"
                    ...
                    @foreach ($bookingOptions as $id => $label)
                        <option value="{{ $id }}" {{ $filterBooking == $id ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
                            ...
                            <td class="px-4 py-4 text-right">
                                <div class="inline-flex items-center gap-2">
                                    <a href="{{ route('marketing.pembayaran.show', $pembayaran->id) }}"
                                        class="font-semibold text-primary transition hover:text-primary-dark" title="Detail">
                                    ...
                                    @if ($pembayaran->bukti_bayar)
                                        <a href="{{ asset('storage/' . $pembayaran->bukti_bayar) }}" target="_blank"
                                            class="font-semibold text-gray-600 transition hover:text-gray-900" title="Lihat Bukti">
                                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
