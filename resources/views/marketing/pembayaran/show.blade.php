    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
            <x-card>
                ...
                <p class="mt-1 text-lg font-semibold text-gray-900 font-mono">
                    {{ $pembayaran->booking?->kode_booking ?? '-' }}</p>
            </x-card>
            <x-card>
            ...
            <x-card>
                ...
                <p class="text-sm text-gray-500">{{ $pembayaran->booking?->unit?->tipe_rumah ?? '' }} -
                    {{ $pembayaran->booking?->unit?->perumahan?->nama_perumahan ?? '' }}</p>
            </x-card>
        </div>
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
            ...
            <x-card class="sm:col-span-2">
                ...
                <p class="mt-1 text-3xl font-bold text-gray-900">Rp {{ number_format($pembayaran->nominal, 0, ',', '.') }}
                </p>
            </x-card>
            ...
            <x-card>
                ...
                <p class="mt-1 text-gray-900">
                    {{ \App\Enums\MetodeBayar::tryFrom($pembayaran->metode_bayar)?->label() ?? '-' }}</p>
            </x-card>
        ...
        @if ($pembayaran->bukti_bayar)
            <x-card title="Bukti Pembayaran">
                ...
                <div class="flex items-center gap-6">
                    @if ($isPdf)
                        <div
                            class="flex h-24 w-24 shrink-0 items-center justify-center rounded-xl border-2 border-dashed border-gray-300 bg-gray-50">
                            <svg class="h-10 w-10 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M7 2h10a2 2 0 012 2v16a2 2 0 01-2 2H7a2 2 0 01-2-2V4a2 2 0 012-2z" />
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h10M7 14h10" />
                            </svg>
                        </div>
                    @else
            ...
            <div class="relative">
                ...
                <ul class="space-y-6">
                    <li class="relative flex items-start gap-4">
                        <div
                            class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-gray-100 ring-2 ring-white">
                    ...
                    <li class="relative flex items-start gap-4">
                        <div
                            class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-amber-100 ring-2 ring-white">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="text-xs text-gray-400">Status: <span
                                    class="font-medium text-amber-800">{{ $pembayaran->status_verifikasi->label() }}</span>
                            </p>
                        </div>
                    </li>
                    ...
                    @if ($pembayaran->status_verifikasi === \App\Enums\StatusVerifikasi::Diverifikasi)
                        <li class="relative flex items-start gap-4">
                            <div
                                class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-emerald-100 ring-2 ring-white">
                    ...
                    @elseif ($pembayaran->status_verifikasi === \App\Enums\StatusVerifikasi::Ditolak)
                        <li class="relative flex items-start gap-4">
                            <div
                                class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-red-100 ring-2 ring-white">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                @if ($pembayaran->tanggal_verifikasi)
                                    <p class="text-xs text-gray-400">
                                        {{ $pembayaran->tanggal_verifikasi->format('d/m/Y H:i') }}</p>
                                @endif
                            </div>
                        </li>
                    @endif
