    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">Perbarui data {{ $user->nama_lengkap }}.</p>
            </div>
        ...
        <x-card title="Informasi Pengguna" subtitle="Kosongkan password apabila tidak ingin menggantinya.">
            <form method="POST" action="{{ route('admin.users.update', $user) }}" enctype="multipart/form-data">
                ...
                <div class="grid gap-x-6 md:grid-cols-2">
                    <x-form-input name="nama_lengkap" label="Nama Lengkap" :value="$user->nama_lengkap" :required="true"
                        maxlength="100" autocomplete="name" />
                    <x-form-input name="username" label="Username" :value="$user->username" :required="true" maxlength="50"
                    ...
                    <x-form-input name="password_confirmation" label="Konfirmasi Password Baru" type="password"
                        placeholder="Ulangi password baru" minlength="8" autocomplete="new-password" />
                    <x-form-input name="email" label="Email" type="email" :value="$user->email" maxlength="100"
                        autocomplete="email" />
                    <x-form-input name="no_hp" label="No. HP" type="tel" :value="$user->no_hp" maxlength="15"
                        autocomplete="tel" />
                    @if ($isCurrentUser)
                    ...
                    @else
                        <x-form-select name="role" label="Role" :options="collect($roles)->mapWithKeys(fn($role) => [$role->value => $role->label()])->all()" :selected="$user->role->value"
                            :required="true" />
                    @endif
                ...
                @if ($user->foto_profil)
                    <div class="mb-4 flex items-center gap-4">
                        <img src="{{ asset('storage/' . $user->foto_profil) }}" alt="{{ $user->nama_lengkap }}"
                            class="h-20 w-20 rounded-xl object-cover">
                        <div>
                            <p class="text-sm text-gray-500">Foto profil saat ini. Upload foto baru untuk menggantinya.</p>
                            <div class="mt-2">
                                <button type="button"
                ...
                @endif
                <x-form-file name="foto_profil" label="Foto Profil Baru" accept="image/jpeg,image/png,image/webp" />
