        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <x-card title="Marketing PIC">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm font-semibold text-gray-900">{{ $booking->marketing?->nama_lengkap ?? '-' }}</p>
            ...
            <x-card title="Riwayat Pembayaran">
                ...
                <div class="mt-4">
                    @include('marketing.booking.riwayat-pembayaran', ['pembayaran' => $booking->pembayaran])
                </div>

                @if($sisaTagihan > 0)
                    <div class="mt-4 flex items-center justify-between rounded-lg bg-amber-50 border border-amber-200 p-3">
                        <div class="text-sm">
                            <span class="text-gray-600">Sisa Tagihan:</span>
                            <span class="ml-2 font-semibold text-gray-900">Rp {{ number_format($sisaTagihan, 0, ',', '.') }}</span>
                        </div>
                    ...
                    <div class="mt-4 rounded-lg bg-amber-50 border border-amber-200 p-3">
                        ...
                        <div class="mt-2 space-y-2">
                            @foreach($pembayaranPending as $p)
                                <div class="flex items-center justify-between rounded bg-white p-2 border border-amber-100">
                                    <div class="text-sm">
                                        <span class="font-medium">{{ $p->jenis_pembayaran->label() }}</span>
                                        <span class="text-gray-500 ml-2">Rp {{ number_format($p->nominal, 0, ',', '.') }}</span>
                                    </div>
            ...
            <x-card title="Catatan">
                ...
                @if($catatanVerifikasi->isNotEmpty())
                    <div class="mt-4 border-t border-gray-100 pt-4">
                        <h4 class="text-xs font-semibold uppercase tracking-wider text-gray-500 mb-2">Catatan Verifikasi</h4>
                        @foreach($catatanVerifikasi as $p)
                            <div class="mb-2 rounded-lg bg-emerald-50 border border-emerald-200 p-3 text-sm">
                                <p class="font-medium text-emerald-800">{{ $p->jenis_pembayaran->label() }} - {{ $p->tanggal_bayar->format('d/m/Y') }}</p>
            ...
            <x-card title="Aksi">
                ...
                @if($statusSekarang === 'booking')
                    <div class="space-y-3">
                        ...
                        @php
                            $docsLengkap = $booking->konsumen?->dokumenKpr->where('status_verifikasi', 'valid')->count() === $booking->konsumen?->dokumenKpr->count()
                        ...
                        @if($booking->pengajuanKpr)
                            <div class="rounded-lg bg-indigo-50 border border-indigo-200 p-4">
                                ...
                                <div class="mt-2 space-y-1 text-sm text-indigo-700">
                                    ...
                                    <p>Plafon: Rp {{ number_format($booking->pengajuanKpr->plafon_kpr ?? 0, 0, ',', '.') }}</p>
                                    ...
                                    <p>Tanggal Pengajuan: {{ $booking->pengajuanKpr->tanggal_pengajuan?->format('d/m/Y') ?? '-' }}</p>
                                </div>
                                <a href="{{ route('admin.pengajuan-kpr.show', $booking->pengajuanKpr->id) }}"
                ...
                @elseif($statusSekarang === 'akad')
                    <div class="rounded-lg bg-blue-50 border border-blue-200 p-4">
                        ...
                        <p class="mt-1 text-sm text-blue-700">Booking telah mencapai tahap akad. Proses penandatanganan akad telah selesai.</p>
                        @if($booking->pengajuanKpr)
                ...
                @elseif($statusSekarang === 'serah_terima')
                    <div class="rounded-lg bg-emerald-50 border border-emerald-200 p-4">
                        ...
                        <p class="mt-1 text-sm text-emerald-700">Unit telah diserahkan ke konsumen. Proses booking telah selesai.</p>
                    </div>
                ...
                @if(in_array($statusSekarang, ['prospek', 'booking', 'pengajuan_kpr', 'akad']))
                    <div class="mt-4 pt-4 border-t border-gray-200">
                        <p class="text-xs font-semibold uppercase tracking-wider text-gray-500 mb-2">Ubah Status Penjualan</p>
...
<div id="modalUbahStatus" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-ubah-status-title" role="dialog" aria-modal="true">
    <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" onclick="document.getElementById('modalUbahStatus').classList.add('hidden')"></div>
                ...
                <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                            <h3 class="text-base font-semibold text-gray-900" id="modal-ubah-status-title">Konfirmasi Ubah Status</h3>
                            <div class="mt-2">
