        ...
        <x-card title="Detail Pembayaran">
            <div class="grid gap-4 text-sm md:grid-cols-2">
                <div>
                    ...
                    <span
                        class="block font-semibold text-gray-900 mt-1">{{ $pembayaran->booking?->konsumen?->nama_lengkap ?? '-' }}</span>
                </div>
                <div>
                    <span class="text-gray-500">Marketing</span>
                    <span
                        class="block font-semibold text-gray-900 mt-1">{{ $pembayaran->booking?->marketing?->nama_lengkap ?? '-' }}</span>
                </div>
                <div>
                ...
                <div>
                    ...
                    <span class="block font-semibold text-gray-900 mt-1">Rp
                        {{ number_format($pembayaran->nominal, 0, ',', '.') }}</span>
                </div>
                <div>
                    <span class="text-gray-500">Tanggal Bayar</span>
                    <span
                        class="block font-semibold text-gray-900 mt-1">{{ $pembayaran->tanggal_bayar->format('d/m/Y') }}</span>
                </div>
                <div>
                    <span class="text-gray-500">Metode</span>
                    <span
                        class="block font-semibold text-gray-900 mt-1">{{ \App\Enums\MetodeBayar::tryFrom($pembayaran->metode_bayar)?->label() ?? '-' }}</span>
                </div>
            </div>
        </x-card>
            ...
            <form action="{{ route('admin.pembayaran.proses-tolak', $pembayaran->id) }}" method="POST" id="formTolak">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        @enderror
                        <p class="mt-1 text-xs text-gray-500">Alasan penolakan wajib diisi dan akan dikirimkan ke marketing.
                        </p>
                    </div>

                    <div class="rounded-lg bg-red-50 border border-red-200 p-4">
                        <div class="flex items-start gap-3">
                            <svg class="h-5 w-5 text-red-600 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
