...
<div class="space-y-6" x-data="verifikasiForm()">
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
        ...
        <x-card>
            ...
            <p class="mt-1 text-2xl font-bold text-gray-900">Rp {{ number_format($pembayaran->nominal, 0, ',', '.') }}</p>
        </x-card>
    </div>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <x-card title="Detail Pembayaran" subtitle="Informasi lengkap pembayaran">
                <div class="grid gap-4 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span class="font-semibold inline-flex items-center rounded-md px-2.5 py-0.5 text-xs border bg-{{ $pembayaran->jenis_pembayaran->color() }}-100 text-{{ $pembayaran->jenis_pembayaran->color() }}-800">
                            {{ $pembayaran->jenis_pembayaran->label() }}
                        </span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-500">Tanggal Bayar</span>
                        <span class="font-semibold text-gray-900">{{ $pembayaran->tanggal_bayar->format('d/m/Y') }}</span>
                    </div>
                    <div class="flex justify-between">
                ...
                <div class="rounded-lg border border-gray-200 bg-gray-50 p-4">
                    ...
                    <div class="grid gap-3 text-sm">
                        <div class="flex justify-between">
                            ...
                            <span class="font-semibold text-gray-900">Rp {{ number_format($pembayaran->nominal, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-500">Nominal yang diharapkan</span>
                            <span class="font-semibold text-gray-900">
                                @if ($pembayaran->jenis_pembayaran === \App\Enums\JenisPembayaran::Dp)
                                    Rp {{ number_format($dpMinNominal, 0, ',', '.') }} ({{ $dpMinPersen }}% dari harga unit)
                        ...
                        @php
                            $diffPersentase = $expectedNominal > 0
                                ? abs((float) $pembayaran->nominal - $expectedNominal) / $expectedNominal * 100
                                : 0;
                        @endphp
                        @if ($diffPersentase > 5)
                            <div class="rounded-lg bg-yellow-50 border border-yellow-200 p-3 text-sm text-yellow-800">
                                ...
                                <span class="font-medium">Peringatan:</span> Nominal yang diinput berbeda >5% dari yang diharapkan.
                            </div>
                        @endif
                    </div>
                </div>
        ...
        @if ($pembayaran->bukti_bayar)
            ...
            <div class="space-y-4">
                <x-card title="Bukti Pembayaran">
                    @if ($isPdf)
                        <div class="flex h-64 w-full items-center justify-center rounded-xl border-2 border-dashed border-gray-300 bg-gray-50">
        ...
        @else
            <div class="flex items-center justify-center rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 h-64">
    ...
    <x-card title="Form Verifikasi" subtitle="Pilih aksi verifikasi untuk pembayaran ini">
        <form action="{{ route('admin.pembayaran.proses-verifikasi', $pembayaran->id) }}" method="POST" id="formVerifikasi">
            @csrf
            @method('PUT')

            <div class="space-y-4">
                <div>
                    <p class="text-sm font-semibold text-gray-700 mb-2">Aksi Verifikasi <span class="text-red-500">*</span></p>
                ...
                <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                            <h3 class="text-base font-semibold text-gray-900" id="modal-title">Konfirmasi Verifikasi</h3>
                            <div class="mt-2">
