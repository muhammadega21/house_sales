        ...
        <div class="flex gap-3">
            ...
            <div class="min-w-0 flex-1">
                ...
                <div class="mt-1">
                    ...
                    <span class="ml-1 inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium bg-{{ $unit->status_unit->color() }}-100 text-{$unit->status_unit->color()}-800">
                        {{ $unit->status_unit->label() }}
                    </span>
                </div>
                <p class="mt-1 text-xs text-gray-500">{{ $unit->perumahan?->nama_perumahan ?? '-' }}</p>
                <p class="text-sm font-semibold text-gray-900 mt-1">Rp {{ number_format($unit->harga_jual, 0, ',', '.') }}</p>
            </div>
        </div>
