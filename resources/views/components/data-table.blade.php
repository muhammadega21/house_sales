...
@php
    ...
    $currentSortDir = request('sort_direction', 'asc');
        ...
        <table class="min-w-full divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    @foreach($columns as $key => $label)
                        ...
                        <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">
                            @if($isSortable)
                                <a href="{{ request()->fullUrlWithQuery(['sort_by' => $key, 'sort_direction' => $currentSortBy === $key ? $nextSortDir : 'asc']) }}" 
                                   class="inline-flex items-center gap-1.5 hover:text-gray-900 group transition-colors duration-200">
                                    {{ $label }}
                                    <span class="text-gray-400 group-hover:text-gray-700 transition-colors duration-200">
                                        @if($currentSortBy === $key)
                                            ...
                                            @endif
                                        @else
                                            <svg class="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <!-- Actions Column -->
                    @if(!empty($actions))
                        <th scope="col" class="relative px-6 py-4 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">
                            Aksi
                        </th>
                    @endif
                </tr>
            ...
            <tbody class="divide-y divide-gray-100 bg-white">
                @forelse($items as $item)
                    <tr class="hover:bg-gray-50/50 transition-colors duration-150" :class="selectedIds.includes({{ $item->id }}) ? 'bg-blue-50/20' : ''">
                        <!-- Checkbox Cell -->
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                ...
                                @elseif($key === 'nama_lengkap')
                                    <div class="flex items-center gap-3">
                                        @if($item->foto_profil)
                                            <img src="{{ asset('storage/' . $item->foto_profil) }}" alt="{{ $item->nama_lengkap }}"
                                                 class="h-9 w-9 rounded-full object-cover ring-2 ring-gray-100">
                                        @else
                                            <div class="flex h-9 w-9 items-center justify-center rounded-full bg-primary/10 text-primary font-semibold text-sm ring-2 ring-gray-100">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="inline-flex items-center gap-3">
                                    @if(in_array('show', $actions) && $routePrefix)
                                        <a href="{{ route($routePrefix . '.show', $item->id) }}" class="text-info hover:text-indigo-800 transition-colors duration-200" title="Detail">
                                    ...
                                    @if(in_array('edit', $actions) && $routePrefix)
                                        <a href="{{ route($routePrefix . '.edit', $item->id) }}" class="text-primary hover:text-primary-dark transition-colors duration-200" title="Edit">
                                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    @if(in_array('delete', $actions) && $routePrefix)
                                        <x-confirm-delete :route="route($routePrefix . '.destroy', $item->id)" :item-name="$item->nama ?? $item->nama_lengkap ?? $item->kode_unit ?? 'item ini'" />
                                    @endif
                                </div>
                            </td>
                        @endif
                        ...
                        <td colspan="{{ count($columns) + (empty($actions) ? 1 : 2) }}" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center justify-center gap-3">
                                ...
                                <div class="max-w-xs">
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">Silakan tambahkan data baru atau sesuaikan filter pencarian Anda.</p>
                                </div>
                            </div>
                        </td>
                    </tr>
